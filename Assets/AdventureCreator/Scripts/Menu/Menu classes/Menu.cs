...
namespace AC
{
	...
	public class Menu : ScriptableObject
	{
		...
		{
			if (appearType == AppearType.WhenSpeechPlays && oneMenuPerSpeech) return;
		...
		public void ShowGUI ()
		{
			...
			else if (appearType == AppearType.WhenSpeechPlays)
			{
				...
				if (/*!oneMenuPerSpeech && */speechMenuType == SpeechMenuType.SpecificCharactersOnly)
				{
					limitToCharacters = EditorGUILayout.TextField ("Character(s) to limit to:", limitToCharacters);
			...
			{
				canClickInCutscene = EditorGUILayout.Toggle ("Clickable in cutscenes?", canClickInCutscene);
			...
			if (menuSource == MenuSource.AdventureCreator)
			{
				...
				{
					autoSizeEveryFrame = EditorGUILayout.Toggle ("Resize every frame?", autoSizeEveryFrame);
		...
		private void TransitionAnimGUI ()
		{
			panMovement = (PanMovement) EditorGUILayout.EnumPopup ("Transition animation:", panMovement);
