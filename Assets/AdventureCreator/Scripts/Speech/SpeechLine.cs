...
namespace AC
{
	...
	public class SpeechLine
	{
			...
			if (this == speechManager.activeLine)
			{
						...
						if (speechManager.translateAudio && textType == AC_TextType.Speech)
						{
							...
							else
							{
								if (i > 0)
								{
									...
									if (speechManager.UseFileBasedLipSyncing ())
									{
										customTranslationLipsyncFiles[i-1] = EditField <Object> ("Lipsync file:", customTranslationLipsyncFiles[i-1]);
									}
									customTranslationAudioClips[i-1] = EditField <AudioClip> ("Audio clip:", customTranslationAudioClips[i-1]);
								}
								else
								{
									if (speechManager.UseFileBasedLipSyncing ())
									{
										customLipsyncFile = EditField <Object> ("Lipsync file:", customLipsyncFile);
									}
									customAudioClip = EditField <AudioClip> ("Audio clip:", customAudioClip);
								}
							}
				...
				if (textType == AC_TextType.Speech)
				{
					...
					if (tagID >= 0 && KickStarter.speechManager.useSpeechTags)
					{
						...
						{
							ShowField ("Tag: ", speechTag.label, false);
						}
					}
				}
		...
		private string EditField (string label, string field, bool multiLine)
		{
			...
			if (multiLine)
			{
				field = EditorGUILayout.TextArea (field, EditorStyles.textArea, GUILayout.MaxWidth (570f));
			}
			else
			{
				field = EditorGUILayout.TextField (field, GUILayout.MaxWidth (570f));
			}
			EditorGUILayout.EndHorizontal ();
			return field;
		}
		...
		private T EditField <T> (string label, T field) where T : Object
		{
			EditorGUILayout.BeginHorizontal ();
			EditorGUILayout.LabelField (label, GUILayout.Width (85f));
			field = (T) EditorGUILayout.ObjectField (field, typeof (T), false, GUILayout.MaxWidth (570f));
			EditorGUILayout.EndHorizontal ();
			return field;
		}
