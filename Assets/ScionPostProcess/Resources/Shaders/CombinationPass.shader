...
{	
	...
	uniform float4 _ThresholdParams;
	#define LensFlareThreshold _ThresholdParams.x 
	#define DirtBloomThreshoold _ThresholdParams.y
	#define BloomThreshold _ThresholdParams.z
	...
	float3 LensFlare(float2 uv)
	{
		...
		lensFlare = lensFlare * starColor;

		//Threshold lensflares
		float lensFlareMax = VectorMax(lensFlare) + 1e-4f;
		float lensFlareMaxAfterThreshold = max(lensFlareMax - LensFlareThreshold, 0.0f);
		float afterThresholdRatio = lensFlareMaxAfterThreshold / lensFlareMax;
		lensFlare = lensFlare * afterThresholdRatio;
		return lensFlare;
	}
	...
	uniform float _SCContrast;
	uniform float _SCMinContrastApplication;
	...
	float3 EnergyPreservingCombineWithThreshold(float3 originalClr, float3 layeredColor, float threshold, float combinationIntensity)
	{
		float layeredMax = VectorMax(layeredColor);
		float layeredMaxAfterThreshold = max(layeredMax - threshold, 0.01f);
		float afterThresholdRatio = layeredMax / layeredMaxAfterThreshold;
		float3 thresholdedLayeredColor = layeredColor * afterThresholdRatio;
		float compensatedCombinationIntensity = saturate(combinationIntensity - afterThresholdRatio + 1.0f);
		return lerp(originalClr, thresholdedLayeredColor, compensatedCombinationIntensity);
	}
	
	float3 GetBloomSamples(float2 screenUV)
	{	
		return tex2Dlod(_BloomTexture, float4(screenUV, 0.0f, 0.0f)).xyz * BloomEnergyNormalization * BloomBrightness;
	}
	
	float3 SampleOnlyBloom(float3 clr, float2 screenUV)
	{
		float3 bloomTexture = GetBloomSamples(screenUV);
		clr = EnergyPreservingCombineWithThreshold(clr, bloomTexture, BloomThreshold, BloomIntensity);	
	...
	float3 SampleBloomDirt(float3 clr, float2 screenUV)
	{
		...
		clr = EnergyPreservingCombineWithThreshold(clr, bloomTexture, BloomThreshold, BloomIntensity);						
		clr = EnergyPreservingCombineWithThreshold(clr, bloomTexture * LensDirtBloomBrightness, DirtBloomThreshoold, saturate(dirtMask*LensDirtBloomEffect));
