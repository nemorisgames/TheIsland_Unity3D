...
namespace ScionEngine
{
	public class DepthOfField
	{
		...
		public void ReleaseResources() 
		{
			...
			{
				#if UNITY_EDITOR
				GameObject.DestroyImmediate(m_maskCamera.gameObject);
				#else
				GameObject.Destroy(m_maskCamera.gameObject);
				#endif
				m_maskCamera = null;
			}
			...
			{
				#if UNITY_EDITOR
				Object.DestroyImmediate(m_DoFMat);
				#else
				Object.Destroy(m_DoFMat);
				#endif
				m_DoFMat = null;
			}

			if (m_DoFMatTemporal != null) 
			{
				#if UNITY_EDITOR
				Object.DestroyImmediate(m_DoFMatTemporal);
				#else
				Object.Destroy(m_DoFMatTemporal);
				#endif
				m_DoFMatTemporal = null;
			}

			// Release texture's held one frame...
			if (previousAlphaTexture != null) 
			{
				RenderTexture.ReleaseTemporary(previousAlphaTexture);
				previousAlphaTexture = null;
			}
			if (previousTapsTexture != null) 
			{
				RenderTexture.ReleaseTemporary(previousTapsTexture);
