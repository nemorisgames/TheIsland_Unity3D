...
namespace ScionEngine
{
	...
	{					
		...
		[Inspector.Field("Threshold", useProperty = "bloomThreshold", visibleCheck = "ShowBloom", tooltip = "This effect will not become visible until a pixel is this bright")]
		[SerializeField] protected float m_bloomThreshold = 0.0f;
		...
		[Inspector.Field("Threshold", useProperty = "lensFlareThreshold", visibleCheck = "ShowLensFlare", tooltip = "This effect will not become visible until a pixel is this bright")]
		[SerializeField] protected float m_lensFlareThreshold = 0.0f;
		...
		[Inspector.Field("Bloom Threshold", useProperty = "lensDirtBloomThreshold", visibleCheck = "ShowLensDirtSettings", tooltip = "This effect will not become visible until a pixel is this bright")]
		[SerializeField] protected float m_lensDirtBloomThreshold = 0.0f;
		...
		[Inspector.Slider("Bloom Brightness Scale", useProperty = "lensDirtBloomBrightness", visibleCheck = "ShowLensDirtSettings", minValue = 0.25f, maxValue = 2.0f, tooltip = "How bright the lens dirt effect is")]
		[SerializeField] protected float m_lensDirtBloomBrightness = 1.2f;
		[Inspector.Slider("Lens Flare Effect", useProperty = "lensDirtLensFlareEffect", visibleCheck = "ShowLensDirtSettings", minValue = 0.0f, maxValue = 1.0f, tooltip = "How modulated the lens flare is by the dirt texture")]
		[SerializeField] protected float m_lensDirtLensFlareEffect = 1.0f;
		[Inspector.Slider("Lens Flare Brightness Scale", useProperty = "lensDirtLensFlareBrightness", visibleCheck = "ShowLensDirtSettings", minValue = 0.25f, maxValue = 2.0f, tooltip = "How bright the lens dirt effect is")]
		...
		protected bool ShowCameraMode() { return unityCamera.hdr == true; }
		...
		{
			get { return m_bloomThreshold; }
			set { value = Mathf.Max(value, 0.0f); m_bloomThreshold = value; postProcessParams.glareParams.threshold = value; }
		}
		...
		{
			get { return m_lensFlareThreshold; }
			set { value = Mathf.Max(value, 0.0f); m_lensFlareThreshold = value; postProcessParams.lensFlareParams.threshold = value; }
		}
		...
		{
			get { return m_lensDirtBloomThreshold; }
			set { value = Mathf.Max(value, 0.0f); m_lensDirtBloomThreshold = value; postProcessParams.lensDirtParams.bloomThreshold = value; }
		}
		...
		{			
			...
			m_cameraTransform		= unityCamera.transform;
		...
		protected void OnDisable() 
		{
			...
			if(m_combinationPass != null)       m_combinationPass.ReleaseResources();
			if(m_depthOfFieldClass != null)     m_depthOfFieldClass.ReleaseResources();
			if(m_downsampling != null)          m_downsampling.ReleaseResources();
			if(m_lensFlareClass != null)        m_lensFlareClass.ReleaseResources();
			if(m_virtualCamera != null)         m_virtualCamera.ReleaseResources();
		...
		{
			focalDistance = focalDistance < unityCamera.nearClipPlane + 0.01f ? unityCamera.nearClipPlane + 0.01f : focalDistance;
			
			postProcessParams.camera = unityCamera;
			...
			if (prevCamFoV != unityCamera.fieldOfView || postProcessParams.preCalcValues.tanHalfFoV == 0.0f)
			{
				postProcessParams.preCalcValues.tanHalfFoV = Mathf.Tan(unityCamera.fieldOfView * 0.5f * Mathf.Deg2Rad);
				prevCamFoV = unityCamera.fieldOfView;
			}
			...
			postProcessParams.cameraParams.fieldOfView	= unityCamera.fieldOfView;
			postProcessParams.cameraParams.aspect		= unityCamera.aspect;
			postProcessParams.cameraParams.nearPlane	= unityCamera.nearClipPlane;
			postProcessParams.cameraParams.farPlane		= unityCamera.farClipPlane;
			
			if (m_isFirstRender == true) postProcessParams.cameraParams.previousViewProjection = unityCamera.projectionMatrix * unityCamera.worldToCameraMatrix;			
			...
			if (m_depthFocusMode == DepthFocusMode.TargetTransform)
			{
				...
				float depth = Mathf.Max(Vector3.Dot(direction, m_cameraTransform.forward), unityCamera.nearClipPlane + 0.01f);
		...
		protected void SetGlobalParameters(PostProcessParameters postProcessParams)
		{
			...
			if (SystemInfo.graphicsDeviceVersion.StartsWith ("Direct") == true) 
			{
				if (unityCamera.actualRenderingPath == RenderingPath.Forward && QualitySettings.antiAliasing > 0) forwardMSAAFix = 1.0f;
			}
		...
		private void Start()
		{
			prevRenderWasHDR = unityCamera.hdr;
		}
		...
		{			
			...
			{
				if (prevRenderWasHDR != unityCamera.hdr) 
				{
					m_isFirstRender = true;
					prevRenderWasHDR = unityCamera.hdr;
				}
		...
		{			
			postProcessParams.cameraParams.previousViewProjection = unityCamera.projectionMatrix * unityCamera.worldToCameraMatrix;
		}
