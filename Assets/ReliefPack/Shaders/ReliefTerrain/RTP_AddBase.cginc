...
#if defined(UNITY_PASS_PREPASSFINAL) || defined(UNITY_PASS_DEFERRED)
...
float2 RTP_CustomTiling;
	...
	void vert (inout appdata v) {
		...
		//
				...
				//	#else
						#ifdef USE_EXTRUDE_REDUCTION
							tHA=saturate(lerp(tex2Dlod(_TERRAIN_HeightMap3, float4(_INPUT_uv.xy,0,detailMIPlevel)), 1, PER_LAYER_HEIGHT_MODIFIER89AB)+0.001);
						#else
	...
	float3 binormalBase = -cross( vertexNorm, tangentBase );
		...
		#define _ddyGlobal float2(0,0)
			...
			float2 _ddyGlobal=ddy(IN_uv_Control);
				...
				float4 tHeightmapTMP = tex2D(_TERRAIN_HeightMap3, tiledUV); // potrzebujemy tego tymczasowo na koncu
			#endif						
		#else
			#ifdef UNITY_PASS_META	
...
//	return;
	...
	float2 dy = ddy( IN_uv);
		...
		//
			...
			#endif
		#else
			float4 splat_controlA = tex2D(_Control3, IN_uv_Control);
		#endif
	...
	mip_selector=0.5*log2(d);
		...
		//triplanar_blend_simple=1;
			...
			float3 microSnowTex = tex2Dgrad(_SparkleMap, uvTRI.xy*rtp_snow_MicroTiling, _ddxMainTRI.xy*rtp_snow_MicroTiling, _ddyMainTRI.xy*rtp_snow_MicroTiling).gba;
		#endif
	#endif	
...
//#endif		 			
			...
			float4 gloss=0;
				...
				if (triplanar_blend_flag) {
					...
					if (triplanar_blend.x>0.02) {
						...
						normals_combined = tex2Dd(_BumpMap89, uvTRI1.xy, _ddxMain.yz*snow_depthMult, _ddyMain.yz*snow_depthMult).rgba*splat_control1.rrgg;
						#if !defined(USE_2_LAYERS_ONLY)
						normals_combined+=tex2Dd(_BumpMapAB, uvTRI1.xy, _ddxMain.yz*snow_depthMult, _ddyMain.yz*snow_depthMult).rgba*splat_control1.bbaa;
						#endif							
					...
					if (triplanar_blend.y>0.02) {
						...
						normals_combined = tex2Dd(_BumpMap89, uvTRI2.xy, _ddxMain.xy*snow_depthMult, _ddyMain.xy*snow_depthMult).rgba*splat_control1.rrgg;
						#if !defined(USE_2_LAYERS_ONLY)
						normals_combined+=tex2Dd(_BumpMapAB, uvTRI2.xy, _ddxMain.xy*snow_depthMult, _ddyMain.xy*snow_depthMult).rgba*splat_control1.bbaa;
						#endif							
					...
					if (triplanar_blend.z>0.02) {
						...
						normals_combined = tex2Dd(_BumpMap89, uvTRI3.xy, _ddxMain.xz*snow_depthMult, _ddyMain.xz*snow_depthMult).rgba*splat_control1.rrgg;
						#if !defined(USE_2_LAYERS_ONLY)
						normals_combined+=tex2Dd(_BumpMapAB, uvTRI3.xy, _ddxMain.xz*snow_depthMult, _ddyMain.xz*snow_depthMult).rgba*splat_control1.bbaa;
						#endif							
					...
					float4 tmp_gloss=0;
						...
						#endif							
					#else
						c = tex2Dd(_SplatC0, uvTRI_tmp.xy, _ddxMainTRI.xy, _ddyMainTRI.xy); col += c.rgb*splat_control1.x; tmp_gloss.r=c.a;
						c = tex2Dd(_SplatC1, uvTRI_tmp.xy, _ddxMainTRI.xy, _ddyMainTRI.xy); col += c.rgb*splat_control1.y; tmp_gloss.g=c.a;
						#if !defined(USE_2_LAYERS_ONLY)
						c = tex2Dd(_SplatC2, uvTRI_tmp.xy, _ddxMainTRI.xy, _ddyMainTRI.xy); col += c.rgb*splat_control1.z; tmp_gloss.b=c.a;
						#if !defined(USE_3_LAYERS_ONLY)
						c = tex2Dd(_SplatC3, uvTRI_tmp.xy, _ddxMainTRI.xy, _ddyMainTRI.xy); col += c.rgb*splat_control1.w; tmp_gloss.a=c.a;
						#endif
						#endif			
					#endif						
					gloss=tmp_gloss;
					...
					float4 normA=tex2Dd(_BumpMap89, uvTRI_tmp.xy, _ddxMainTRI.xy*snow_depthMult, _ddyMainTRI.xy*snow_depthMult).rgba;
					float4 normals_combined = normA*splat_control1.rrgg;
					#if !defined(USE_2_LAYERS_ONLY)
					float4 normB=tex2Dd(_BumpMapAB, uvTRI_tmp.xy, _ddxMainTRI.xy*snow_depthMult, _ddyMainTRI.xy*snow_depthMult).rgba;
			...
			col*=dot(splat_control1, _LayerBrightness89AB);  
				...
				rayPos.w=mip_selector.x+rtp_mipoffset_bump;
					...
					rayPos.w += snow_depth;
				#endif				
				normals_combined = tex2Dd(_BumpMap89, rayPos.xy, _ddxMain.xy*snow_depthMult, _ddyMain.xy*snow_depthMult).rgba*splat_control1.rrgg;
				#if !defined(USE_2_LAYERS_ONLY)
				normals_combined+=tex2Dd(_BumpMapAB, rayPos.xy, _ddxMain.xy*snow_depthMult, _ddyMain.xy*snow_depthMult).rgba*splat_control1.bbaa;
				#endif							
				n.xy=(normals_combined.rg+normals_combined.ba)*2-1;
				n.xy*=_uv_Relief_z;
				n.z = sqrt(1 - saturate(dot(n.xy, n.xy)));
...
//#endif
			// snow color
				...
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_0)
					half4 csnow = tex2Dlod(_SplatC0, rayPos.xyww+SNOW_PM_OFFSET);
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_5)
				...
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_1)
					half4 csnow = tex2Dlod(_SplatC1, rayPos.xyww+SNOW_PM_OFFSET);
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_6)
				...
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_2)
					half4 csnow = tex2Dlod(_SplatC2, rayPos.xyww+SNOW_PM_OFFSET);
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_7)
				...
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_3)
					half4 csnow = tex2Dlod(_SplatC3, rayPos.xyww+SNOW_PM_OFFSET);
					GETrtp_snow_TEX
				#endif
			#endif	
			#endif
...
//#endif		
	...
	float _BumpMapGlobalStrengthPerLayer=1;
		...
		// UBER - micro bumps and dissolve
		#if !defined(RTP_TRIPLANAR)
			float3 microSnowTex = tex2Dgrad(_SparkleMap, IN_uv_Relief_Offset.xy*rtp_snow_MicroTiling, _ddxMain.xy*rtp_snow_MicroTiling, _ddyMain.xy*rtp_snow_MicroTiling).gba;
		#endif
		...
		col=lerp( col, rtp_snow_color_tex, snow_val );
			...
			float3 n;
				...
				n.xy=tex2Dlod(_BumpMap89, float4(IN_uv_Relief_Offset.xy+SNOW_PM_OFFSET.xy, mip_selector + rtp_mipoffset_bump)).rg*2-1;
			#endif
			#ifdef RTP_SNW_CHOOSEN_LAYER_NORM_1
				n.xy=tex2Dlod(_BumpMap89, float4(IN_uv_Relief_Offset.xy+SNOW_PM_OFFSET.xy, mip_selector + rtp_mipoffset_bump)).ba*2-1;
			#endif
			#ifdef RTP_SNW_CHOOSEN_LAYER_NORM_2
				n.xy=tex2Dlod(_BumpMapAB, float4(IN_uv_Relief_Offset.xy+SNOW_PM_OFFSET.xy, mip_selector + rtp_mipoffset_bump)).rg*2-1;
			#endif
			#ifdef RTP_SNW_CHOOSEN_LAYER_NORM_3
				n.xy=tex2Dlod(_BumpMapAB, float4(IN_uv_Relief_Offset.xy+SNOW_PM_OFFSET.xy, mip_selector + rtp_mipoffset_bump)).ba*2-1;
			#endif
